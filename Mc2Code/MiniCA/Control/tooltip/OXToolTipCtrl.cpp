...
BOOL COXToolTipCtrl::HitTest(CWnd *pWnd, POINT pt, OXTOOLINFO* pToolInfo) const
{
	...
	for (int i = 0; i < nSize; i++)
	{
		...
		if ((pWnd == m_pParentWnd) && pTool->rectBounds.PtInRect(pt))
		{
			if (pToolInfo) *pToolInfo = *pTool;
			bFoundToolWnd = TRUE;
			break;
		}
		if ((pWnd != m_pParentWnd) && (pWnd->GetSafeHwnd() == pTool->hWnd ||
			::IsChild(pTool->hWnd, pWnd->GetSafeHwnd())))
		{
			if (pToolInfo)
				*pToolInfo = *pTool;
...
void COXToolTipCtrl::UpdateTipText(UINT nIDText,CWnd* pWnd,UINT_PTR nIDTool /*=0*/)
{
	CString str;
	str.LoadString(nIDText);
...
void COXToolTipCtrl::UpdateTipText(LPCTSTR lpszText, CWnd* pWnd, UINT_PTR nIDTool /*=0*/)
{
	COXToolTipInfo* pToolInfo = GetToolInfoPtr(pWnd, nIDTool);
	if (pToolInfo)
	{
		if (lpszText == LPSTR_TEXTCALLBACK)
		{
			pToolInfo->szText = lpszText;
			pToolInfo->strText.Empty();
		}
		else
		{
			pToolInfo->szText = NULL;
			pToolInfo->strText = lpszText;
		}
	}
} 
