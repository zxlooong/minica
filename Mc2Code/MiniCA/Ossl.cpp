// Ossl.cpp: implementation of the COssl class.
//
//////////////////////////////////////////////////////////////////////
...
void COssl::LastError(char * info)
{
...
X509 *COssl::load_cert(BIO * pBioCert/*输入BIO*/, const int iFormat/*格式*/,
...
X509 * COssl::LoadCert(const char * lpszCert,const int iCertlen,
...
EVP_PKEY * COssl::load_key(BIO * pBioKey, const int iFormat, const char * lpszPass,char * outMsg)//枚举DER/PEM格式
{
	EVP_PKEY * pkey = NULL;
	OpenSSL_add_all_algorithms();
...
EVP_PKEY * COssl::LoadKey(const char * lpszkey,const int iKeylen,const char * lpszPass,char * outMsg) //leak 4772
{
	EVP_PKEY *pkey = NULL;
	BIO * in = NULL;
...
int COssl::Rand(const char * file,const int dont_warn,char * outMsg)//产生随机数,return 0 ---成功
{
...
*/
int COssl::Add_ExtCert(X509 * pCert/*正被添加的证书*/,
...
int COssl::AddExtCert(X509 *pCert/*正被添加的证书*/,
			   X509 * pRoot/*根证书（从中得到信息)*/, 
...
void COssl::Ansi2Utf8(const LPSTR lpsrc, const int srclen, LPSTR lpdst, int& dstlen)
{
	WCHAR * pwUnicode;      
...
BOOL COssl::Add_Name(X509_NAME * px509Name,
			  const int iNid/*c\cn*/,
...
void COssl::AddName(X509_NAME * px509Name,const stuSUBJECT * pSubJect)
{
	setlocale(LC_CTYPE, "");
...
/*将BIO转换未内存,和长度*/
BOOL COssl::Bio2Mem(BIO * memBio/*[IN]*/,char * lpszMem/*[OUT]*/,
...
/*X509转换为Mem*/
BOOL COssl::X5092Mem(X509 *px509/*[IN]*/, const int iType /*[in]类型pem-der*/,
...
/*EVP_PKEY转换为BIO*/
BOOL COssl::Key2Mem(EVP_PKEY * pkey/*[IN]*/,const int iType/*[in]类型pem-der*/,
...
BOOL COssl::CertPair2XE(const stuCertPair & RootPair/*[IN]*/,X509 *& pCert, EVP_PKEY *& pKey ,char * outMsg)
{
	pCert = NULL;
	pKey = NULL;
	//检测stuCertPair结构合法性
...
//在内存中创建PFX
BOOL COssl::CreateMemPfx(X509 * pCert, EVP_PKEY * key, const  char * p12Pwd, const char * friendlyName,
...
BOOL COssl::mkRoot(const stuSUBJECT *rootInfo,
			X509 ** x509p/*out公钥*/, 
...
BOOL COssl::MakeRoot(const stuSUBJECT * rootInfo,/*[in]信息*/
 ...
 * because we wont reference any other sections.
 */
int COssl::Add_ExtReq(STACK_OF(X509_REQUEST) *sk,const int iNid,const char *lpszValue)
{
	X509_EXTENSION *ex;
...
int COssl::mkReq(const stuSUBJECT * reqInfo,
		  X509_REQ **req,
		  EVP_PKEY **pkeyp, 
		  int bits,
		  char * outMsg)
...
BOOL COssl::MakeReq(const stuSUBJECT * reqInfo,/*请求信息IN*/
			 const int bits/*位数IN*/,
...
////////////////////////////////////////////////////////////////////
/////////////////////////// begin ////////////////////////////////////////
int COssl::copy_extensions(X509 *x, X509_REQ *req, const int copy_type)//在证书中加入req自带扩展信息
{
	STACK_OF(X509_EXTENSION) * exts = NULL;
...
int COssl::do_body(X509 **xret, EVP_PKEY *pkey, X509 *px509, 
...
int COssl::certify(X509 **xret, X509_REQ *req, EVP_PKEY *pkey, 
...
BOOL COssl::MakeCert(const stuCertPair & RootPair/*根证书对*/,
			  const long serial/*序列号*/,
...
///////////////////////// end ////////////////////////////////////////
BOOL COssl::DirectCert(const stuCertPair &RootPair /*根证书对*/,
				const int bits /*[IN]*/,
				const long serial /*[IN]序列号*/,
...
//////////////////////////////////////////////////////////////////////
///////////////////////// begin //////////////////////////////////////
int COssl::Add_ExtCrl(X509_CRL *crl/*正被添加的证书*/,X509 * pRoot/*根证书（从中得到信息)*/, 
			   const int iNid, const char *lpszValue)
{
	X509V3_CTX ctx;
	X509_EXTENSION *ex;
...
BOOL COssl::MakeCrl(const stuCertPair & RootPair/*根证书对*/,
...
BOOL COssl::CertFormatConver(const char * buf/*文件内容或文件名称*/,
...
//分解p12包
BOOL COssl::ParsePfx(const char * strP12/*包文件或内存*/,
...
//组合p12包
BOOL COssl::CreatePfx(char * StreamP12/*OUT包文件路径或内存区域*/,
...
//修改p12包密码
BOOL COssl::ChangePfxPwd(const char * strP12/*in老包文件*/,
...
//检验公钥、私钥是否配对
BOOL COssl::CertPairCheck(const char * pCert,
				   const char * key,
				   char * outMsg,
...
int COssl::HexToTen(const char * pHex)
{
    DWORD dwHexNum=0;
    for (; *pHex!=0 ; pHex++)
    {
...
void COssl::Utf8ToAnsi(const UCHAR * lpsrc,const int srclen, LPSTR lpdst, int& dstlen)
{
	WCHAR * Unicode;
...
BOOL COssl::CheckCertWithCrl(const char *pubCert,const int pubCertLen,
...
BOOL COssl::CheckCertWithRoot(const char *pubCert,const int pubCertLen,
...
BOOL COssl::CheckCertLife(const char *pubCert,const int pubCertLen,char * outMsg)
{
	X509 *x509 = LoadCert(pubCert,pubCertLen,NULL,outMsg);
	if (x509 == NULL)
	{
...
/*组合P7包*/
BOOL COssl::CreateP7b(std::list<std::string> * pCertList,
			   const char * lpszCrl,
			   const char * outP7b,
			   const int outformat,
			   char * outMsg/*返回结果*/)
...
/*分解P7包*/
BOOL COssl::ParseP7b(const char * lpszInP7b/*包文件或内存*/,
			  const UINT iP12Len/* 如果包为文件,则为0,否则为内存长度*/,
			  const int outformat/*输出格式*/,
...
//转换证书到REQ
BOOL COssl::X5092Req(char * Cert/*公钥*/,int Certlen,
